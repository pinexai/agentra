...
from agentra.guard.attacks import PLUGIN_REGISTRY, AttackPlugin
...
class RedTeamReport:
    ...
    def summary(self) -> None:
        print(f"\n{'='*60}")
        print(f"agentra Red Team Report — {self.target_fn}")
        print(f"{'='*60}")
            ...
            "runs": [
                {
                    "tool": {
                        "driver": {
                            "name": "agentra",
                            "version": __version__,
                            "informationUri": "https://github.com/pinexai/agentra",
                            "rules": rules,
                        }
                    },
    ...
    def save_sarif(self, path: str) -> None:
        ...
        print(f"[agentra] SARIF report saved to {path}")
    ...
    def to_junit(self) -> str:
        ...
        lines = [
            ...
            f'<testsuite name="agentra" tests="{self.total_attacks}" failures="{failures}" '
        ...
        for plugin, results in by_plugin.items():
            ...
            severity = _SEVERITY_MAP.get(plugin, "UNKNOWN")
            lines.append(
                f'  <testcase name="{plugin}" classname="agentra.{plugin}" '
                f'time="{duration:.3f}">'
            )
            if plugin_failures:
    ...
    def save_junit(self, path: str) -> None:
        ...
        print(f"[agentra] JUnit XML report saved to {path}")
    ...
    def _persist(self) -> None:
        try:
            ...
            print(f"[agentra] Warning: could not persist report: {e}")
...
def _judge_response(
    ...
    """
    from agentra.providers import call as _call
    from agentra.pricing import calculate
...
def red_team(
    ...
    from agentra.providers import get_judge_model
    ...
    for p in plugins:
        if isinstance(p, str):
            ...
            if cls is None:
                print(f"[agentra] Warning: unknown plugin '{p}', skipping.")
                continue
    ...
    print(f"\n[agentra] Red teaming {fn_name!r} with {len(plugin_instances)} plugin(s), {n_attacks} attacks each...")
    ...
    for plugin_name, plugin in plugin_instances:
        ...
        if use_mutations and len(attacks) >= 2:
            try:
                from agentra.guard.mutations import get_mutated_attacks
                n_base = n_attacks // 2
    ...
    if git_compare:
        from agentra.git_tracker import compare_scans
        ...
        if fail_on_regression and comparison.has_regression:
            print(f"\n[agentra] FAIL: vulnerability rate increased by {comparison.delta:+.1%} vs {git_compare}")
            sys.exit(1)

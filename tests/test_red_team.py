"""Tests for sentrix.guard.red_team — severity, SARIF, JUnit, cost guardrails, multi-layer judge."""
import importlib
...
class TestSeverityTiers:
    ...
    def test_hallucination_is_medium(self):
        from sentrix.guard.red_team import _SEVERITY_MAP
...
class TestSARIF:
    ...
    def test_sarif_rules_per_plugin(self):
        ...
        assert "sentrix/jailbreak" in rule_ids
    ...
    def test_sarif_results_for_vulnerable(self):
        ...
        assert results[0]["ruleId"] == "sentrix/jailbreak"
...
class TestCostGuardrail:
    def test_max_cost_stops_early(self):
        ...
        def judge_with_cost(attack, response, model):
            nonlocal call_count
            call_count += 1
...
class TestMultiLayerJudge:
    def test_keyword_prefilter_returns_safe_without_llm(self):
        ...
        with patch("sentrix.providers.call") as mock_call:
    ...
    def test_ambiguous_response_calls_llm(self):
        from sentrix.guard.red_team import _judge_response
        with patch("sentrix.providers.call") as mock_call, \
             patch("sentrix.pricing.calculate") as mock_calc:

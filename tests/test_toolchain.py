"""Tests for sentrix.guard.toolchain — privilege escalation mapper."""
import importlib
from unittest.mock import patch

import pytest

_tc_mod = importlib.import_module("sentrix.guard.toolchain")
...
class TestToolCategorization:
    def test_reader_detected(self):
        from sentrix.guard.toolchain import _categorize_tool
...
class TestToolchainReport:
    def _make_report(self):
        from sentrix.guard.toolchain import ToolchainReport
    ...
    def test_persist(self, tmp_db):
        from sentrix.db import _q, set_db_path
        report = self._make_report()
        set_db_path(tmp_db)
        report._persist()
...
class TestScanToolchain:
    ...
    def test_chain_pattern_detection(self):
        from sentrix.guard.toolchain import _chain_matches_pattern

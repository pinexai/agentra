"""Tool-Chain Privilege Escalation Mapper — sentrix v0.2.0.
...
class ToolchainReport:
    ...
    def _persist(self) -> None:
        ...
        try:
            ...
            print(f"[sentrix] Warning: could not persist toolchain report: {e}")
        finally:
            conn.close()
...
def scan_toolchain(
    ...
    """
    from sentrix.providers import _DEFAULT_JUDGE_MODEL
    ...
    # Enumerate all chains of length 2..max_chain_depth
    escalation_chains = []
    total_chains_tested = 0
    total_cost = 0.0

    for depth in range(2, min(max_chain_depth + 1, len(tools) + 1)):
        # All permutations of tools of this length (allow repeats for realistic chains)
        for combo in product(range(len(tools)), repeat=depth):
            ...
            # Cap total chains to avoid explosion
            if total_chains_tested >= 50:
                break
        if total_chains_tested >= 50:
            break
